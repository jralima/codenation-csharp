using System;
using Xunit;

namespace Codenation.Challenge
{
    public class FIFACupStatsTest
    {
        [Fact]
        public void Shoud_Return_20_Itens_When_Get_Top_Players()
        {
            var cup = new FIFACupStats();
            var topPlayers = cup.First20Players();
            Assert.NotNull(topPlayers);
            Assert.Equal(20, topPlayers.Count);
        }

        [Fact]
        public void Shoud_Return_10_Itens_When_Get_Top_Players_By_Release_Clause()
        {
            var cup = new FIFACupStats();
            var topPlayers = cup.Top10PlayersByReleaseClause();
            Assert.NotNull(topPlayers);
            Assert.Equal(10, topPlayers.Count);
        }

        [Fact]
        public void Shoud_Return_10_Itens_When_Get_Top_Players_By_Age()
        {
            var cup = new FIFACupStats();
            var topPlayers = cup.Top10PlayersByAge();
            Assert.NotNull(topPlayers);
            Assert.Equal(10, topPlayers.Count);
        }

        [Fact]
        public void Shoud_Return_Age_Count_Map_When_Get_Players_By_Age()
        {
            var cup = new FIFACupStats();
            var dicPlayers = cup.AgeCountMap();
            Assert.NotNull(dicPlayers);
            Assert.Equal(29, dicPlayers.Count);
        }
              
        [Fact]
        public void Shoud_Return_Count_Nationality()
        {
            var cup = new FIFACupStats();
            var nationalityDistinctCount = cup.NationalityDistinctCount();
            Assert.True(nationalityDistinctCount == 164);
        }

        [Fact]
        public void Shoud_Return_Count_Club_Distinct()
        {
            var cup = new FIFACupStats();
            var clubDistinctCount = cup.ClubDistinctCount();
            Assert.True(clubDistinctCount == 648);
        }

    }
}
